...
class PicMagnifierWidget extends YouYaX
{
	...
	public function judge(){
		...
		if(substr_count($arg_list[0],"<img src=")){
			if(preg_match_all("/<img src=\"([^<]*?upload[^<]*?)\">/",$arg_list[0],$tmp)){
				foreach($tmp[1] as $k => $v){
					...
					if($pic_t[0] > 794){
						$arg_list[0] = str_replace($tmp[0][$k],"<a target='_blank' href='".$v."'><img style='cursor:url(".C('SITE')."/Public/images/magplus.gif),pointer;border:none;' width=794 src='".$v."'></a>",$arg_list[0]);
					}
				}
				return $arg_list[0];
			}else{
