...
class MessageAction extends YouYaX
{
    public function send()
    {
        ...
        if (!empty($_SESSION['youyax_user']) && CommonAction::competence($_SESSION['youyax_user'])) {
            ...
            if ($_SESSION['youyax_user'] == $mto) {
                ...
                echo '收信人不能为空';
                exit;
            }else if(!$this->find(C('db_prefix') . "user", 'string', "user='" . $mto . "'")){
           	 echo '用户不存在';
               exit;
            }else {
                $data          = array();
    ...
    public function show()
    {
        ...
        $sql   = "select * from " . C('db_prefix') . "message where mto='" . $_SESSION['youyax_user'] . "' order by id desc";
        ...
        $this->assign('data', $result)->assign('page', $show)->assign('site', C('SITE'))->assign('user', $user)->assign('shtml', C('static_url'))->assign('url', C('default_url'));
        $mix = require("./Conf/mix.config.php");
        $this->assign('mix', $mix);
    ...
    public function delMess()
    {
        ...
        if ($result) {
            ...
            $this->assign('jumpurl', $this->youyax_url . "/Message" . C('default_url') . "show" . C('static_url'))->assign('msgtitle', '操作失败')->assign('message', '请仔细检查！')->error();
        }
    }
