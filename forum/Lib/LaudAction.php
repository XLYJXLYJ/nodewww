    ...
    public function dolaud()
    {
        ...
        if (!empty($_SESSION['youyax_user'])) {
            ...
            if (empty($_POST['types'])) {
                if ($ips_arr = $this->find(C('db_prefix') . "talk", "string", "id=" . $tid . " and is_question=1")) {
                    if ($ips_arr['zuozhe'] != $_SESSION['youyax_user']) {
                        ...
                        if (in_array($myIp, $ips)) {
                            ...
                            if (intval($_POST['bz']) == 1) {
                                $data['laud'] = $laud + 1;
                                mysql_query("update " . C('db_prefix') . "user set bid=bid+1 where user='" . $ips_arr['zuozhe'] . "'");
                            } else {
                                $data['laud'] = $laud - 1;
                                mysql_query("update " . C('db_prefix') . "user set bid=bid-1 where user='" . $ips_arr['zuozhe'] . "'");
                            }
                            $this->save($data, C('db_prefix') . "talk", "id=" . $tid);
                            echo 'success';
                            exit;
                        }
                    } else {
                ...
                if ($ips_arr = $this->find(C('db_prefix') . "reply", "string", "id2=" . intval($_POST['val2']) . " and rid=" . $tid)) {
                    if ($ips_arr['zuozhe1'] != $_SESSION['youyax_user']) {
                        ...
                        if (in_array($myIp, $ips)) {
                            ...
                            if (intval($_POST['bz']) == 1) {
                                $data['laud2'] = $laud + 1;
                                mysql_query("update " . C('db_prefix') . "user set bid=bid+1 where user='" . $ips_arr['zuozhe1'] . "'");
                            } else {
                                $data['laud2'] = $laud - 1;
                                mysql_query("update " . C('db_prefix') . "user set bid=bid-1 where user='" . $ips_arr['zuozhe1'] . "'");
                            }
                            $this->save($data, C('db_prefix') . "reply", "id2=" . intval($_POST['val2']));
                            echo 'success';
                            exit;
                        }
                    } else {
