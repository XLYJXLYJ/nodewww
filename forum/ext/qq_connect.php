...
header("Content-type:text/html;charset=utf-8");
  ...
  if(!empty($user->openid)){
     ...
     if ($row_num == 0){
     		$sql_user = "SELECT * FROM ".$config['db_prefix']."user where user='".$nick."'";
     		$res_user = mysql_query($sql_user);
     		$row_num_user = mysql_num_rows($res_user);
...
$face="upload/".$time.".jpg";
       	 $sql="insert into ".$config['db_prefix']."user(user,pass,status,face,bid,user_group,openid,time) values('".$nick."','".md5("")."','3','".$face."','100','".$config['default_user_group']."','".$user_id."',now())";
      	 mysql_query($sql);
   ...
   mysql_query("update " . $config['db_prefix'] . "user set cookieid='".$cookieid."' where user='".$_SESSION['youyax_user']."'");
